...
def build_report_template():
    ...
    # Clean up JS
    js = remove_wasm_code(js)
    js = remove_filter_code(js)
    js = remove_file_input_code(js)
...
<style>{css_min}</style>
<style>
/* Hide filter bar and file input in report mode */
.filter-bar, #fileInputSection {{ display: none !important; }}
#app {{ margin-top: 0; }}
</style>
</head>
<body>
...
if (COMPRESSED_DATA) {{
    ...
    decompressData(COMPRESSED_DATA).then(data => {{
        ...
        renderResults(data, 'Report', 0, true);
    }}).catch(err => {{
